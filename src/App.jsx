  ...
  const handleFile = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // ИСПРАВЛЕНИЕ: Добавлен ключ 'data' перед вызовом функции
      ...
      if (requests.length > 0) {
        ...
        excelData.sheets.forEach((sheet, index) => {
          if (index === 0) {
            ...
            if (reply && reply.addSheet) sheetIds[index] = reply.addSheet.properties.sheetId
            replyIndex++
          }
        })
      } else {
          ...
          {step === 4 && result && (
            <div className="text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center mb-6">
                <a href={result.url} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center bg-[#1D6F42] hover:bg-[#165232] text-white font-medium py-3 px-6 rounded-lg transition-colors shadow-md">
                  <img src="/logoGS.png" alt="GS" className="w-5 h-5 mr-2" />
                  Открыть Google Таблицу <ArrowRightIcon className="ml-2 w-4 h-4"/>
                </a>
                
                <button onClick={downloadExcel} className="inline-flex items-center justify-center bg-[#1D6F42] hover:bg-[#165232] text-white font-medium py-3 px-6 rounded-lg transition-colors shadow-md">
